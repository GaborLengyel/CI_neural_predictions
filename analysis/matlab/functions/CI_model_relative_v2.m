function [neuron_resps_group,resps_group] = CI_model_relative_v2( dir_c, dir_s,speed_c,speed_s,n_trials,subid,ndots,model_params,neuronType,causalStruct)
...
if ~exist('fr_joint','var')
    ...
    elseif linear == 2
        ...
        %fr_joint = @(nu) abs(neuron_speed_tuning(nu, R0, A, alpha, tau, n));
    else
        mu_ = neuronType{2};
        kappa_ = neuronType{3};
        alpha_ = neuronType{4};
...
%neuron_resps_group=fr_joint(speed_resps_group);
end
