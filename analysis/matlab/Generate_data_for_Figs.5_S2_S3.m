...
%             logSpd];
c_speed = speed_vec;
s_speed = [0,n1_m-(n1_sd/2.2), n1_m-(n1_sd/4), n1_m, n1_m+n1_sd, n1_m+(2*n1_sd);...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%startS = 101; endS=200;
%meanfire = zeros(lenS,len_s_speed,len_c_speed,len_s,len_c);
varfire = zeros(length(neuronTypes),lenS,len_s_speed,len_c_speed,len_s,len_c);
%varfire = zeros(lenS,len_s_speed,len_c_speed,len_s,len_c);
%recent%meanfire = zeros(length(neuronTypes),lenS*lenV,lenNS,len_s_speed,len_c_speed,len_s,len_c);
%meanfire = zeros(length(neuronTypes),lenS,lenNS,len_s_speed,len_c_speed,len_s,len_c);
%k=1;l=31;
parfor NT=1:length(neuronTypes)
    neuronType = neuronTypes{NT};
    for MP=1:5%:NM % loop over subject parameters
        disp([NT,MP])
        for NL=1%:lenNS
            %NT = 5;
            %neuronType = neuronTypes{NT};
            mdp = model_params(MP,NL,:);
                ...
                for j=1:len_c_speed % speed center
                    for k = 1:len_s % surround direction
                        for l = 1:len_c % center direction
                            if varType==1
                                [fr,~]=CI_model_percept_v2(c_dir(l), s_dir(k), c_speed(NT,j), s_speed(NT,i),num_trials,1,ndots,mdp,neuronType,causalStruct(1));
                            elseif varType==2
                                [fr,~]=CI_model_retinal_v2(c_dir(l), s_dir(k), c_speed(NT,j), s_speed(NT,i),num_trials,1,ndots,mdp,neuronType,causalStruct(1));
                            elseif varType==3
                                [fr,~]=CI_model_relative_v2(c_dir(l), s_dir(k), c_speed(NT,j), s_speed(NT,i),num_trials,1,ndots,mdp,neuronType,causalStruct(1));
                            elseif varType==4
                                [fr,~]=CI_model_group_v2(c_dir(l), s_dir(k), c_speed(NT,j), s_speed(NT,i),num_trials,1,ndots,mdp,neuronType,causalStruct(1));
                            end
                            %meanfire(NT,MP,NL,i,j) = mean(fr);
                            %meanfire(NT,MP,NL,i,j,k,l) = mean(fr);
                            varfire(NT,MP,i,j,k,l) = var(fr);
                            %varfire(MP,i,j,k,l) = var(fr);
                        end
                    end
                end
            end
...
save('VF_5subj_full_retina.mat','varfire',"-v7.3")
